                                 ...
                                 char* out, size_t out_len)
{
    ...
    for (const mbedtls_x509_name* cur = name; cur != NULL; cur = cur->next) {

        /* CN OID = 2.5.4.3 */
        if (MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &cur->oid) == 0) {

            /* cur->val.p is NOT null-terminated */
            size_t n = cur->val.len;
            if (n >= out_len) n = out_len - 1;

            memcpy(out, cur->val.p, n);
            out[n] = '\0';
            return 0;
        }
    }
...
static int extract_cn_from_x509_name_der(const unsigned char* der, size_t der_len,
                                        char* out, size_t out_len)
{
    ...
    if (mbedtls_asn1_get_tag((unsigned char**)&p, end, &len,
        ...
        return -1;
    const unsigned char* name_end = p + len;

    while (p < name_end) {

        /* RDN ::= SET OF AttributeTypeAndValue */
        if (mbedtls_asn1_get_tag((unsigned char**)&p, name_end, &len,
                                MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SET) != 0)
            return -1;
        const unsigned char* rdn_end = p + len;

        while (p < rdn_end) {

            /* AttributeTypeAndValue ::= SEQUENCE { type OID, value ANY } */
            if (mbedtls_asn1_get_tag((unsigned char**)&p, rdn_end, &len,
                                    MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE) != 0)
                return -1;
            const unsigned char* atv_end = p + len;

            /* type: OID */
            mbedtls_asn1_buf oid;
            oid.tag = *p;
            if (mbedtls_asn1_get_tag((unsigned char**)&p, atv_end, &len,
                                    MBEDTLS_ASN1_OID) != 0)
                return -1;
            oid.len = len;
            oid.p = (unsigned char*)p;
            p += len;

            /* value: we accept PrintableString, UTF8String, IA5String */
            int is_cn = MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &oid) == 0;

            if (p >= atv_end) return -1;
            unsigned char val_tag = *p;

            if (mbedtls_asn1_get_tag((unsigned char**)&p, atv_end, &len, val_tag) != 0)
                return -1;

            if (is_cn) {
                /* Copy value bytes into out */
                size_t cpy = (len < out_len - 1) ? len : (out_len - 1);
                memcpy(out, p, cpy);
                out[cpy] = '\0';
                return 0; /* CN found */
            }
    ...
    while (ext_len != 0) {
        ...
        switch (ext_id) {
            case MBEDTLS_TLS_EXT_TRUSTED_CA_KEYS:
                ret = ssl_parse_trusted_ca_keys( ssl, ext + 4, ext_size );
                if (ret != 0) {
                    return ret;
                }
                break;
